...
class Dreamer(RlAlgorithm):
    ...
    def __init__(
            ...
            # batch_length=40,
            # train_every=200,
            train_steps=100, # temp make it to 10.
            pretrain=100,
            model_lr=6e-4,
            value_lr=8e-5,
            actor_lr=8e-5,
    ...
    def write_videos(self, observation, action, image_pred, post, step=None, n=4, t=25):
        ...
        concat = torch.cat((reconstruction, imagined), dim=0) + 0.5
        error = (concat - ground_truth + 1) / 2
        # concatenate vertically on height dimension
        openl = torch.cat((ground_truth, concat, error), dim=3)
        openl = openl.transpose(1, 0)  # N,T,C,H,W
        self.vidwrite(f'vid-{step}.mp4', openl[0].cpu(), 1)
        self.vidwrite2(f'vid2-{step}.mp4', openl[0].cpu())
    ...
    def vidwrite2(self, fn, images, framerate=1, vcodec='libx264'):
        if not isinstance(images, np.ndarray):
            images = np.asarray(images)
            images = images.transpose((0, 3, 2, 1))
        ...
        video = VideoWriter(fn, fourcc, float(framerate), (width, height))
